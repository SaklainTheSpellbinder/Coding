...
class CFT2D:
    ...
    def compute_cft(self):
        ...
        cosx=np.cos(2*np.pi*np.outer(self.u,self.x))
        sinx=np.sin(2*np.pi*np.outer(self.u,self.x))
        cosy=np.cos(2*np.pi*np.outer(self.v,self.y))
        siny=np.sin(2*np.pi*np.outer(self.v,self.y))

        #tintinc=self.I[:,None,:]*cosx[None,:,:]
        cx=np.trapezoid(self.I[:,None,:]*cosx[None,:,:],self.x,axis=2)
        #tintins=self.I[:,None,:]*sinx[None,:,:]
        sx=np.trapezoid(self.I[:,None,:]*sinx[None,:,:],self.x,axis=2)

        #real_tintin=cx[None,:,:]*cosy[:,:,None]-sx[None,:,:]*siny[:,:,None]
        real=np.trapezoid(cx[None,:,:]*cosy[:,:,None]-sx[None,:,:]*siny[:,:,None],self.y,axis=1)
        #imag_tintin=-sx[None,:,:]*cosy[:,:,None]-cx[None,:,:]*siny[:,:,None]
        imag=np.trapezoid(-sx[None,:,:]*cosy[:,:,None]-cx[None,:,:]*siny[:,:,None],self.y,axis=1)
        return real, imag
    ...
    def plot_magnitude(self):
        ...
        plt.imshow(np.log(1+magnitude),cmap='grey')
        plt.show()
...
class InverseCFT2D:
    ...
    def reconstruct(self):
        ...
        a=np.trapezoid((self.real[:,:,None]*cosy[:,None,:])-(self.imag[:,:,None]*siny[:,None,:]),self.v,axis=0)
        b=np.trapezoid((self.real[:,:,None]*siny[:,None,:])+(self.imag[:,:,None]*cosy[:,None,:]),self.v,axis=0)
        I=np.trapezoid((a[:,:,None]*cosx[:,None,:]-b[:,:,None]*sinx[:,None,:]),self.u,axis=0)
        return I
